...
var Map = function (game) {
	...
	this.tiles = [];
	this.pixitiles = new PIXI.Container(); //We can avoid removing all the entities with the sprites
	this.addChild(this.pixitiles);
...
Map.prototype.updateAllSprites = function () { //Update all sprites via first removing them all
	...
	for (var x = 0; x < this.settings.tilesX; x++) { for (var y = 0; x < this.settings.tilesY; y++) { //Loop through every tile
		...
		//If it has both, add it, otherwise leave it be
		flag |= (flag&bits.n+bits.w==bits.n+bits.w)?bits.nw:0;
		flag |= (flag&bits.n+bits.e==bits.n+bits.w)?bits.ne:0;
		flag |= (flag&bits.s+bits.w==bits.n+bits.w)?bits.sw:0;
		flag |= (flag&bits.s+bits.e==bits.n+bits.w)?bits.se:0;
		...
		switch (flag & 0xf) { //Only the last four bits--this is important for the inside corners
			...
			case bits.s+bits.n+bits.w+bits.e:
				//Inside corner bits
				var f = flag & (0xf0);
				//It doesn't have NW, so it's a NW inside corner
				if (f==bits.nw)	nw[nw.length]=this.tiles[x][y];
				if (f==bits.ne)	ne[ne.length]=this.tiles[x][y];
				if (f==bits.sw)	sw[sw.length]=this.tiles[x][y];
				if (f==bits.se)	se[se.length]=this.tiles[x][y];
				if (f==0)	i[i.length]=this.tiles[x][y]; //If it's not missing any then it's completely surrounded, a.k.a. inside tile
				break;
		}
	}}
}
